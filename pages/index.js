  ...
  return (
    <div style={{ animation: "fadeIn 0.3s ease" }}>
      <div style={{ textAlign: "center", marginBottom: 20 }}>
        ...
        <h3 style={{ fontFamily: "Syne, sans-serif", fontSize: 18, fontWeight: 800, color: C.black, marginBottom: 5 }}>Your assessment is ready.</h3>
      ...
      <button onClick={() => onSubmit({ email, wa, method })} disabled={!ok || loading} style={{
        ...
        border: "none", borderRadius: 8, fontFamily: "Syne, sans-serif", fontSize: 14, fontWeight: 700,
        color: "#fff", cursor: !ok || loading ? "default" : "pointer",
        display: "flex", alignItems: "center", justifyContent: "center", gap: 8,
      }}>
        {loading ? <><span className="qf-spinner" />Generating...</> : <>Show My Results</>}
      </button>
      <p style={{ fontFamily: "Figtree, sans-serif", fontSize: 11, color: C.greyLight, marginTop: 8, textAlign: "center" }}>No spam. Just the assessment and a follow-up if we can help.</p>
    </div>
  );
}
  ...
  return (
    <div style={{ animation: "fadeIn 0.4s ease" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, paddingBottom: 12, borderBottom: `1.5px solid ${C.greyLight}` }}>
        <div style={{ display: "flex", alignItems: "center", gap: 7 }}><Bolt size={16} /><span style={{ fontFamily: "Syne, sans-serif", fontSize: 14, fontWeight: 800, color: C.black, textTransform: "uppercase" }}>QuickFund</span></div>
      ...
      {sections.map((s, i) => { const tag = tm[s.type] || tm.plain; return (
        <div key={i} style={{ marginBottom: 8, padding: s.type === "plain" ? "4px 0" : "11px 12px", background: tag.bg, borderRadius: 8 }}>
          {tag.l && <div style={{ fontFamily: "Syne, sans-serif", fontSize: 9, fontWeight: 800, color: tag.c, letterSpacing: "1.5px", marginBottom: 3 }}>{tag.l}</div>}
      ...
      <a href="https://wa.me/6580576702?text=Hi%20QuickFund%2C%20just%20did%20the%20AI%20assessment%20and%20would%20like%20to%20discuss" target="_blank" rel="noopener" style={{
        ...
        fontFamily: "Syne, sans-serif", fontSize: 14, fontWeight: 700, color: "#fff", textAlign: "center", textDecoration: "none",
  ...
  return (
    <div>
      ...
      <div style={{opacity:fade?0:1,transform:fade?"translateX(-8px)":"none",transition:"all 0.2s"}}>
        <div style={{fontFamily:"Syne, sans-serif",fontSize:10,fontWeight:700,color:C.orange,textTransform:"uppercase",letterSpacing:"2px",marginBottom:6}}>{step+1}/{STEPS.length}</div>
        <h3 style={{fontFamily:"Syne, sans-serif",fontSize:17,fontWeight:700,color:C.black,marginBottom:14,lineHeight:1.25}}>{cur.q}</h3>
      ...
      <div style={{display:"flex",justifyContent:"space-between",marginTop:16}}>
        ...
        {last&&ans[cur.id]&&<button onClick={()=>setGate(true)} style={{padding:"10px 20px",background:C.black,border:"none",borderRadius:7,fontFamily:"Syne, sans-serif",fontSize:12,fontWeight:700,color:"#fff",cursor:"pointer",display:"flex",alignItems:"center",gap:6}}>Get Assessment <Bolt size={12}/></button>}
      </div>
    ...
    try {
      const r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},
        body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,
          system:`QuickFund Term Sheet Scanner. Part of the Levara Advisory Group. We review loan terms for business borrowers across 8 markets.
...
Format: "RED FLAG:" serious, "AMBER:" negotiate/clarify, "OK:" standard, "MISSING:" protections that should exist, "BOTTOM LINE:" overall. Always end with "QuickFund can review this in detail with you. WhatsApp +65 8057 6702."`,
  ...
  return (<div>
    ...
    <button onClick={()=>{if(terms.trim().length<30){setErr("Need more detail.");return;}setGate(true);}} disabled={terms.trim().length<30} style={{width:"100%",padding:"12px",marginTop:12,background:terms.trim().length<30?C.greyLight:C.black,border:"none",borderRadius:7,fontFamily:"Syne, sans-serif",fontSize:13,fontWeight:700,color:"#fff",cursor:terms.trim().length<30?"default":"pointer"}}>Scan My Terms</button>
    {err&&<p style={{fontFamily:"Figtree, sans-serif",fontSize:12,color:C.orange,marginTop:8}}>{err}</p>}
  </div>);
}
...
function RateCheck() {
  ...
  const submit = async(contact) => {
    ...
    try {
      const r=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},
        body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,
          system:`QuickFund Rate Check. Part of the Levara Advisory Group. Benchmark business loan costs in Singapore and across 8 markets.
...
Format: "GOOD:" competitive for the product type, "WATCH:" above typical range for the product type, "VERDICT:" overall assessment, "NEXT:" what to do. Always end with "QuickFund can explore alternatives for your situation. WhatsApp +65 8057 6702."`,
  ...
  return (<div>
    ...
    <button onClick={()=>setGate(true)} disabled={!ok} style={{width:"100%",padding:"12px",marginTop:8,background:!ok?C.greyLight:C.black,border:"none",borderRadius:7,fontFamily:"Syne, sans-serif",fontSize:13,fontWeight:700,color:"#fff",cursor:!ok?"default":"pointer"}}>Check My Rate</button>
    {err&&<p style={{fontFamily:"Figtree, sans-serif",fontSize:12,color:C.orange,marginTop:8}}>{err}</p>}
  </div>);
}
    ...
    <div style={{display:"flex",gap:0,marginBottom:20,borderBottom:`2px solid ${C.greyLight}`}}>
      {[{id:"checkup",l:"New Financing"},{id:"scan",l:"Review Terms"},{id:"rate",l:"Am I Overpaying?"}].map(t=>(
        <button key={t.id} onClick={()=>setTab(t.id)} style={{
          ...
          cursor:"pointer",marginBottom:-2,fontFamily:"Syne, sans-serif",fontSize:11,fontWeight:700,
  ...
  return(
    <div onClick={onClose} style={{position:"fixed",inset:0,zIndex:200,background:"rgba(0,0,0,0.5)",backdropFilter:"blur(8px)",display:"flex",alignItems:"center",justifyContent:"center",padding:20,animation:"fadeIn 0.2s ease"}}>
      <div onClick={e=>e.stopPropagation()} style={{background:C.offwhite,borderRadius:14,padding:"28px 26px",maxWidth:400,width:"100%",position:"relative"}}>
        ...
        <div style={{fontFamily:"Syne, sans-serif",fontSize:10,fontWeight:800,color:C.orange,letterSpacing:"2px",textTransform:"uppercase",marginBottom:8}}>Free Guide</div>
        <h3 style={{fontFamily:"Syne, sans-serif",fontSize:20,fontWeight:800,color:C.black,lineHeight:1.15,marginBottom:8}}>Before You Take The Loan</h3>
        ...
        <button onClick={()=>{console.log("Guide lead:",{wa,email,method});setDone(true);}} disabled={!ok} style={{width:"100%",padding:"12px",marginTop:12,background:!ok?C.greyLight:C.orange,border:"none",borderRadius:7,fontFamily:"Syne, sans-serif",fontSize:13,fontWeight:700,color:"#fff",cursor:!ok?"default":"pointer"}}>Send Me The Guide</button>
      </div>
    </div>);
}
...
function SectorGuideModal({sector, onClose}){
  ...
  return(
    <div onClick={onClose} style={{position:"fixed",inset:0,zIndex:200,background:"rgba(0,0,0,0.5)",backdropFilter:"blur(8px)",display:"flex",alignItems:"center",justifyContent:"center",padding:20,animation:"fadeIn 0.2s ease"}}>
      <div onClick={e=>e.stopPropagation()} style={{background:C.offwhite,borderRadius:14,padding:"28px 26px",maxWidth:400,width:"100%",position:"relative"}}>
        <button onClick={onClose} style={{position:"absolute",top:12,right:12,background:"none",border:"none",fontSize:18,color:C.grey,cursor:"pointer"}}>x</button>
        <div style={{fontFamily:"Syne, sans-serif",fontSize:10,fontWeight:800,color:C.orange,letterSpacing:"2px",textTransform:"uppercase",marginBottom:8}}>Free Sector Guide</div>
        <h3 style={{fontFamily:"Syne, sans-serif",fontSize:20,fontWeight:800,color:C.black,lineHeight:1.15,marginBottom:8}}>
...
export default function QF() {
  ...
  const scrollTo=id=>{document.getElementById(id)?.scrollIntoView({behavior:"smooth",block:"start"});setMenu(false);};
  ...
  return (
    <div style={{background:C.offwhite,minHeight:"100vh",overflowX:"hidden"}}>
      ...
      <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:100,background:scrollY>40?"rgba(250,248,245,0.97)":"transparent",backdropFilter:scrollY>40?"blur(16px)":"none",borderBottom:scrollY>40?"1px solid rgba(0,0,0,0.04)":"none",transition:"all 0.3s",padding:"0 20px"}}>
        <div style={{maxWidth:1100,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",height:58}}>
          <div style={{display:"flex",alignItems:"center",gap:7}}>
            <Bolt size={16}/>
            <span style={{fontFamily:"Syne, sans-serif",fontSize:15,fontWeight:800,color:C.black,textTransform:"uppercase",letterSpacing:"-0.3px"}}>QuickFund</span>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:20}}>
            ...
            <a href="https://wa.me/6580576702?text=Hi%20QuickFund" target="_blank" rel="noopener" style={{fontFamily:"Syne, sans-serif",fontSize:11,fontWeight:700,color:"#fff",background:C.black,padding:"8px 16px",borderRadius:6,textDecoration:"none",textTransform:"uppercase",letterSpacing:"0.5px"}}>WhatsApp</a>
      ...
      <div className={`qf-mobile-menu ${menu?"open":""}`} style={{display:"none",position:"fixed",top:58,left:0,right:0,background:C.offwhite,padding:"20px 24px",borderBottom:`1px solid ${C.greyLight}`,zIndex:99,flexDirection:"column",gap:14,animation:"fadeIn 0.2s ease"}}>
        <a onClick={()=>scrollTo("tools")} style={{fontFamily:"Syne, sans-serif",fontSize:15,fontWeight:700,color:C.black,cursor:"pointer"}}>AI Tools</a>
        <a onClick={()=>scrollTo("partners")} style={{fontFamily:"Syne, sans-serif",fontSize:15,fontWeight:700,color:C.black,cursor:"pointer"}}>Partners</a>
        <a onClick={()=>scrollTo("faq")} style={{fontFamily:"Syne, sans-serif",fontSize:15,fontWeight:700,color:C.black,cursor:"pointer"}}>FAQ</a>
        <a href="https://quickfund.sg/blog/" style={{fontFamily:"Syne, sans-serif",fontSize:15,fontWeight:700,color:C.black,textDecoration:"none"}}>Blog</a>
        <a href="https://wa.me/6580576702?text=Hi%20QuickFund" target="_blank" rel="noopener" style={{fontFamily:"Syne, sans-serif",fontSize:14,fontWeight:700,color:"#fff",background:C.orange,padding:"12px 0",borderRadius:8,textDecoration:"none",textAlign:"center",marginTop:4}}>WhatsApp Us</a>
      </div>
      ...
      <section style={{paddingTop:100,paddingBottom:40}} className="qf-w">
        <div className="qf-hero-grid" style={{display:"grid",gridTemplateColumns:"1fr 440px",gap:40,alignItems:"start"}}>
          ...
          <div>
            ...
            <h1 className="qf-hero-h1" style={{fontFamily:"Syne, sans-serif",fontSize:52,fontWeight:800,color:C.black,lineHeight:0.95,letterSpacing:"-2.5px",marginBottom:20}}>
            ...
            <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:16}}>
              <a href="https://wa.me/6580576702?text=Hi%20QuickFund" target="_blank" rel="noopener" style={{fontFamily:"Syne, sans-serif",fontSize:13,fontWeight:700,color:"#fff",background:C.orange,padding:"12px 24px",borderRadius:8,textDecoration:"none",display:"inline-flex",alignItems:"center",gap:7}}>
                WhatsApp Us Now
              </a>
              <a onClick={()=>scrollTo("tools")} style={{fontFamily:"Syne, sans-serif",fontSize:13,fontWeight:700,color:C.black,background:"transparent",padding:"12px 24px",borderRadius:8,textDecoration:"none",border:`1.5px solid ${C.greyLight}`,cursor:"pointer"}}>
                Try the AI Tools
              </a>
            </div>
            ...
            <div style={{marginTop:28,padding:"14px 18px",background:C.cream,borderRadius:8,borderLeft:`3px solid ${C.orange}`}}>
              <p style={{fontFamily:"Figtree, sans-serif",fontSize:12,color:C.black,lineHeight:1.5,margin:0,fontWeight:500}}>
                Trusted by listed companies across Nasdaq, SGX, Thailand SET, and London AIM, Series A and B startups, VCs, funds, and family offices across 8 markets.
              </p>
            </div>
        ...
        <div className="qf-3col" style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:3}}>
          {[
            ...
            <div key={i} style={{background:p.bg,padding:"28px 24px",borderRadius:2}}>
              <h3 style={{fontFamily:"Syne, sans-serif",fontSize:16,fontWeight:800,color:p.dark?"#fff":C.black,marginBottom:8}}>{p.t}</h3>
      ...
      {/* ── SECTOR GUIDES ── */}
      <section style={{paddingTop:40,paddingBottom:50}} className="qf-w">
        <div style={{fontFamily:"Syne, sans-serif",fontSize:10,fontWeight:800,color:C.orange,letterSpacing:"2px",textTransform:"uppercase",marginBottom:10}}>Sector Guides</div>
        <h2 style={{fontFamily:"Syne, sans-serif",fontSize:22,fontWeight:800,color:C.black,lineHeight:1.1,letterSpacing:"-1px",marginBottom:8}}>Every industry finances differently.</h2>
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
          {[
            ...
            <a key={i} href={`https://quickfund.sg/guide/${sector.toLowerCase().replace(/ and /g,"-").replace(/ /g,"-")}`}
              style={{
      ...
      <section style={{background:C.black,padding:"48px 20px"}}>
        <div style={{maxWidth:1100,margin:"0 auto"}}>
          <div className="qf-2col" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:40,alignItems:"center"}}>
            <div>
              <div style={{fontFamily:"Syne, sans-serif",fontSize:10,fontWeight:800,color:C.orange,letterSpacing:"2px",textTransform:"uppercase",marginBottom:12}}>Why us</div>
              <h2 style={{fontFamily:"Syne, sans-serif",fontSize:28,fontWeight:800,color:"#fff",lineHeight:1.1,letterSpacing:"-1.5px",marginBottom:14}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}} className="qf-stat-grid">
              {[{n:"8",l:"Markets across Asia, US, and UK"},{n:"3 days",l:"Average time to first offer"},{n:"$0",l:"Upfront for financing introductions"},{n:"24/7",l:"We answer your WhatsApp"}].map((s,i)=>(
                <div key={i} style={{padding:20,background:"rgba(255,255,255,0.04)",borderRadius:8,border:"1px solid rgba(255,255,255,0.06)"}}>
                  <div style={{fontFamily:"Syne, sans-serif",fontSize:28,fontWeight:800,color:C.orange,lineHeight:1}}>{s.n}</div>
      ...
      <section id="partners" style={{paddingTop:50,paddingBottom:50}} className="qf-w">
        <div style={{background:"#fff",borderRadius:12,padding:"clamp(24px,4vw,40px)",border:`1.5px solid ${C.greyLight}`}}>
          <div className="qf-2col" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:32,alignItems:"center"}}>
            <div>
              <div style={{fontFamily:"Syne, sans-serif",fontSize:10,fontWeight:800,color:C.orange,letterSpacing:"2px",textTransform:"uppercase",marginBottom:10}}>Referral Partners</div>
              <h2 style={{fontFamily:"Syne, sans-serif",fontSize:26,fontWeight:800,color:C.black,lineHeight:1.1,letterSpacing:"-1.5px",marginBottom:12}}>
              ...
              <a href="https://wa.me/6580576702?text=Hi%20QuickFund%2C%20interested%20in%20partnership" target="_blank" rel="noopener" style={{fontFamily:"Syne, sans-serif",fontSize:13,fontWeight:700,color:"#fff",background:C.black,padding:"12px 24px",borderRadius:8,textDecoration:"none",display:"inline-block"}}>Become a Partner</a>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {[
                ...
                <div key={i} style={{padding:"10px 14px",background:C.offwhite,borderRadius:8,display:"flex",gap:10,border:`1px solid ${C.greyLight}`}}>
                  ...
                  <div>
                    <div style={{fontFamily:"Syne, sans-serif",fontSize:12,fontWeight:700,color:C.black}}>{item.l}</div>
      ...
      <section id="faq" style={{paddingBottom:50}} className="qf-w">
        <div style={{fontFamily:"Syne, sans-serif",fontSize:10,fontWeight:800,color:C.orange,letterSpacing:"2px",textTransform:"uppercase",marginBottom:10}}>FAQ</div>
        <h2 style={{fontFamily:"Syne, sans-serif",fontSize:26,fontWeight:800,color:C.black,lineHeight:1.1,letterSpacing:"-1.5px",marginBottom:24}}>The stuff you actually want to know.</h2>
        <div className="qf-faq-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:2}}>
          {[
            ...
            {q:"How do I know I can trust you?",a:"Registered Singapore company. We've worked with listed companies across Nasdaq, SGX, Thailand SET, and London AIM, as well as Series A and B startups, VCs, funds, and family offices across 8 markets. Happy to provide references."},
          ].map((item,i)=>(
            <details key={i} style={{padding:"14px 18px",background:i%2===0?"#fff":C.offwhite,borderRadius:2,cursor:"pointer"}}>
              <summary style={{fontFamily:"Syne, sans-serif",fontSize:14,fontWeight:700,color:C.black,listStyle:"none",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
      ...
      <section style={{paddingBottom:50}} className="qf-w">
        <div style={{background:C.cream,borderRadius:12,padding:"clamp(28px,4vw,40px)",borderLeft:`4px solid ${C.black}`}}>
          <div style={{fontFamily:"Syne, sans-serif",fontSize:10,fontWeight:800,color:C.black,letterSpacing:"2px",textTransform:"uppercase",marginBottom:10}}>Responsible Borrowing</div>
          <h2 style={{fontFamily:"Syne, sans-serif",fontSize:"clamp(22px,3vw,28px)",fontWeight:800,color:C.black,lineHeight:1.1,letterSpacing:"-1px",marginBottom:14}}>
            We'd rather lose a deal than<br/>put you in a position you can't<br/>recover from.
          </h2>
          <p style={{fontFamily:"Figtree, sans-serif",fontSize:14,color:C.grey,lineHeight:1.7,marginBottom:16,maxWidth:560}}>
          ...
          <p style={{fontFamily:"Figtree, sans-serif",fontSize:14,color:C.grey,lineHeight:1.7,marginBottom:20,maxWidth:560}}>
          ...
          <p style={{fontFamily:"Figtree, sans-serif",fontSize:13,color:C.grey,lineHeight:1.5,marginTop:16,maxWidth:560}}>
      ...
      <section style={{paddingBottom:60}} className="qf-w">
        <div style={{background:C.black,borderRadius:14,padding:"clamp(32px,5vw,48px) clamp(24px,4vw,40px)",textAlign:"center",position:"relative",overflow:"hidden"}}>
          ...
          <div style={{position:"relative"}}>
            <h2 style={{fontFamily:"Syne, sans-serif",fontSize:"clamp(22px,3.5vw,34px)",fontWeight:800,color:"#fff",lineHeight:1.1,letterSpacing:"-1.5px",marginBottom:10}}>
            ...
            <div style={{display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap"}}>
              <a href="https://wa.me/6580576702?text=Hi%20QuickFund" target="_blank" rel="noopener" style={{padding:"14px 32px",background:C.orange,color:"#fff",borderRadius:8,fontFamily:"Syne, sans-serif",fontSize:14,fontWeight:700,textDecoration:"none"}}>WhatsApp Us</a>
              <a onClick={()=>scrollTo("tools")} style={{padding:"14px 32px",background:"transparent",color:"rgba(255,255,255,0.5)",border:"1.5px solid rgba(255,255,255,0.12)",borderRadius:8,fontFamily:"Syne, sans-serif",fontSize:14,fontWeight:700,textDecoration:"none",cursor:"pointer"}}>Try the AI Tools</a>
            </div>
          </div>
        </div>
      </section>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:24,marginBottom:28}} className="qf-3col">
            <div>
              ...
              {["Home","AI Tools","Partners","FAQ","Blog","Responsible Business Promise"].map(l=>(
                <a key={l} href={l==="Home"?"https://quickfund.sg":l==="Blog"?"https://quickfund.sg/blog/":`https://quickfund.sg/${l.toLowerCase().replace(/ /g,"-")}`} style={{display:"block",fontFamily:"Figtree, sans-serif",fontSize:12,color:"rgba(255,255,255,0.35)",textDecoration:"none",marginBottom:5}}>{l}</a>
              ))}
            </div>
            <div>
              ...
              {["Property-Backed Loans","Doctors and Dentists","Trading Companies","Tech Companies","Startups","E-commerce","Software","F&B","Construction","Logistics","Professional Services","Retail"].map(l=>(
                <a key={l} href={`https://quickfund.sg/guide/${l.toLowerCase().replace(/ /g,"-")}`} style={{display:"block",fontFamily:"Figtree, sans-serif",fontSize:12,color:"rgba(255,255,255,0.35)",textDecoration:"none",marginBottom:5}}>{l}</a>
              ))}
            </div>
            <div>
