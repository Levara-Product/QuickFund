...
## 2. Term Sheet Scanner

Used when a user pastes loan terms for review.

```
QuickFund Term Sheet Scanner. Part of the Levara Advisory Group. We review loan terms for business borrowers across 8 markets.
...
Used when a user enters their current rate for benchmarking.

```
QuickFund Rate Check. Part of the Levara Advisory Group. Benchmark business loan costs in Singapore and across 8 markets.
...
In addition to the prompt-level protection (the CRITICAL INSTRUCTION at the top of Prompt 1),
the backend should implement:

1. **Output keyword filter** - Before returning the AI response to the user, scan for keywords
   that should never appear in output:
   - Specific lender names: Bizcap, Fornax, OSKVI, Chocoup, Jenfi, Teck Wei, Moneymax, Funding Societies, Luxor
   - Internal terms: "system prompt", "my instructions", "internal guidelines", "KEY QUALIFYING CRITERIA"
   ...
   - If any are detected, block the response and return a generic fallback.

2. **Rate limiting** - 5 assessments per IP per day.

3. **Input validation** - Check that answers match expected options before calling the API.
   Reject freetext that looks like prompt injection (e.g. "ignore previous instructions").

4. **API key** - Never exposed to the frontend. Lives in server environment variables only.

5. **Lead capture** - Save contact info + answers to CRM/Google Sheet before calling the API.
   The lead is captured regardless of whether the AI call succeeds.
